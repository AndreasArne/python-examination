...
class ExamTestCase(unittest.TestCase):
    ...
    def _set_test_name_and_assignment(self):
        ...
        try:
            ...
            raise TestClassNameError(
                "Class name for TestCase should follow the structure 'Test[<number>]<words>'. Got '"
                + test_string
                + "'"
            ) from e
        ...
        try:
            self.test_name = (
                re.search(self.TEST_NAME_REGEX, test_string).group(2).replace("_", " ")
            )
        except AttributeError as e:
            raise TestFuncNameError(
                "Test function name should follow the structure 'test[_<letter>]_<name>' Got '"
                + test_string
                + "'"
            ) from e
        ...
        if module_path is None:
            ...
            try:
                ...
                msg = self._formatMessage(
                    msg, f"{module} not found in path {module_path}"
                )
    ...
    def assertAttribute(self, obj, attr, msg=None):
        ...
        try:
            ...
            msg = self._formatMessage(
                msg, f"attribute {attr} not found in object {obj}"
            )
            raise self.failureException(msg) from e
    ...
    def assertNotAttribute(self, obj, attr, msg=None):
        ...
        try:
            getattr(obj, attr)
            msg = self._formatMessage(
                msg, f"attribute {attr} was found in object {obj}"
            )
            raise self.failureException(msg)
    ...
    def assertOrder(self, order, container, msg=None):
        ...
        try:
            ...
            msg = self._formatMessage(
                msg,
                f"Index of elemnts in {order} don't appear in correct order in {container}",
            )
            raise self.failureException(msg) from e
